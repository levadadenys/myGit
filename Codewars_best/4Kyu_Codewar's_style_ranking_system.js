class User {
    ...
    incProgress(taskLvl) {
        let rankIndex = this._ranks.indexOf(this.rank) + 1;
        let taskIndex = this._ranks.indexOf(taskLvl) + 1;
        if (rankIndex && taskIndex) {
            let difference = taskIndex - rankIndex;
            let gainedExp = 0;

            if (difference > 0) {
                gainedExp = 10 * (difference) * (difference);
            ...
            this._checkProgress(gainedExp);
        } else throw new Error('No such rank');
    }
    _checkProgress(exp) {
        ...
        if (this.progress >= 100) {
            this.rank = this._ranks[(this._ranks.indexOf(this.rank)) + 1];
...
// A user's rank progress starts off at zero, each time the progress reaches 100 the user's rank 
...
//  (we don't throw any progress away). The exception is if there is no other rank left to progress 
...
// It uses a pure Math based solution that gives consistent results no matter what order a set 
